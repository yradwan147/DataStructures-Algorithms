...
for call in calls:
    if call[0].startswith('(080)'):
        if call[1].startswith('(0'):
            called_codes.append(
                call[1][call[1].find('('): call[1].find(')')+1])
        elif call[1][0] in ['7', '8', '9']:
            called_codes.append(call[1][:4])
        elif call[1].startswith('140'):
            called_codes.append('140')
# print(called_codes)

print('The numbers called by people in Bangalore have codes:')
called_codes_nodup = set(called_codes)
for code in sorted(called_codes_nodup):
    print(code)
